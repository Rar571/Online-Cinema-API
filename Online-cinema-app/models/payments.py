...
class PaymentModel(Base):
    ...
    user: Mapped["UserModel"] = relationship(
        "UserModel", back_populates="user_payments"
    )
    order_id: Mapped[int] = mapped_column(
        ForeignKey("orders.id", ondelete="CASCADE"), nullable=False
    )
    order: Mapped["OrderModel"] = relationship(
        "OrderModel", back_populates="order_payments"
    )
    ...
    payment_items: Mapped["PaymentItemModel"] = relationship(
...
class PaymentItemModel(Base):
    ...
    order_item: Mapped["OrderItemModel"] = relationship(
        "OrderItemModel", back_populates="order_payment_items"
    )
